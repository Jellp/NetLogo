...
public strictfp class Patch
  ...
  public Object getPatchVariable(int vn) {
    if (vn == VAR_PCOLOR &&
        _variables[VAR_PCOLOR] == null) {
      _variables[VAR_PCOLOR] = Double.valueOf(pcolor);
    }
  ...
  public double pcolorDouble() {
    if (_variables[VAR_PCOLOR] == null || _variables[VAR_PCOLOR] instanceof Double) {
      return pcolor;
    }
  ...
  public void pcolor(double pcolor) {
    ...
    if (this.pcolor != pcolor) {
      this.pcolor = pcolor;
      _variables[VAR_PCOLOR] = null;
      _world.patchColors()[(int) id()] = Color.getARGBbyPremodulatedColorNumber(pcolor);
  ...
  public void pcolorDoubleUnchecked(Double boxedColor) {
    ...
    if (color != pcolor) {
      pcolor = color;
      _variables[VAR_PCOLOR] = boxedColor;
      _world.patchColors()[(int) id()] = Color.getARGBbyPremodulatedColorNumber(color);
      _world.markPatchColorsDirty();
      if (color != 0.0) {
        _world.patchesAllBlack(false);
      }
  ...
  public void pcolor(LogoList rgb, int varIndex, boolean allowAlpha)
    ...
    if (!(_variables[varIndex] instanceof LogoList) || !rgb.equals(_variables[varIndex])) {
      ...
      _world.patchColors()[(int) id()] = Color.getRGBInt(((Double) rgb.get(0)).intValue(),
