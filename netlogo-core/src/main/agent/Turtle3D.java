...
public final strictfp class Turtle3D
  ...
  void initvars(Number xcor, Number ycor, AgentSet breed) {
    ...
    variables[VAR_SHAPE3D] = _world.turtleBreedShapes().breedShape(breed);
  ...
  public Patch getPatchAtOffsets(double dx, double dy)
    ...
    Patch target = ((World3D) _world).getPatchAt(xcor + dx, ycor + dy, zcor);
    if (target == null) {
  ...
  public Patch3D getPatchAtOffsets(double dx, double dy, double dz)
      throws AgentException {
    Patch3D target = ((World3D) _world).getPatchAt(xcor + dx, ycor + dy, zcor + dz);
    if (target == null) {
      throw new AgentException("Cannot get patch beyond limits of current world.");
    }
    return target;
  ...
  public Patch getPatchHere() {
    if (currentPatch == null) {
      ...
      currentPatch = ((World3D) _world).getPatchAtWrap(xcor, ycor, zcor);
    }
    return currentPatch;
  }
  ...
  public void pitch(double pitch) {
    ...
    if (this == _world.observer().targetAgent()) {
      _world.observer().updatePosition();
    }
  ...
  void drawLine(double x0, double y0, double z0, double x1, double y1, double z1) {
    if (penMode().equals(PEN_DOWN) && (x0 != x1 || y0 != y1 || z0 != z1)) {
      ((World3D) _world).drawLine(x0, y0, z0, x1, y1, z1, color(), penSize());
    }
  }
  ...
  public void moveToPatchCenter() {
    ...
    if (x != oldX || y != oldY || z != oldZ) {
      ...
      if (_world.tieManager().hasTies()) {
        ((TieManager3D) _world.tieManager()).turtleMoved(this, x, y, z, oldX, oldY, oldZ);
      }
    }
  }
  ...
  public void xcor(double xcor)
    ...
    this.xcor = _world.topology().wrapX(xcor);
  ...
  public void xcor(Double xcor)
    ...
    double wrapped = _world.topology().wrapX(x);
    double oldX = this.xcor;
  ...
  public void ycor(double ycor)
    ...
    this.ycor = _world.topology().wrapY(ycor);
  ...
  public void ycor(Double ycor)
    ...
    double wrapped = _world.topology().wrapY(y);
  ...
  public void xandycor(double xcor, double ycor)
    ...
    double oldX = this.xcor;
    double oldY = this.ycor;
    double newX = _world.topology().wrapX(xcor);
    double newY = _world.topology().wrapY(ycor);
  ...
  public void xandycor(Double xcor, Double ycor)
    ...
    double wrappedX = _world.topology().wrapX(x);
    double wrappedY = _world.topology().wrapY(y);
  ...
  public void face(Agent agent, boolean wrap) {
    ...
    try {
      newHeading = _world.protractor().towards(this, agent, wrap); // true = wrap
    } catch (AgentException ex) {
      newHeading = heading();
    }
    try {
      newPitch = _world.protractor().towardsPitch(this, agent, wrap);
    } catch (AgentException ex) {
      newPitch = pitch();
    }
  ...
  public void face(double x, double y, double z, boolean wrap) {
    ...
    try {
      newHeading = _world.protractor().towards(this, x, y, wrap);
    } catch (AgentException ex) {
    ...
    try {
      newPitch = _world.protractor().towardsPitch(this, x, y, z, wrap);
    } catch (AgentException ex) {
      org.nlogo.api.Exceptions.ignore(ex);
    }
  ...
  public Patch getPatchAtHeadingAndDistance(double delta, double distance)
    ...
    return ((Protractor3D) _world.protractor()).getPatchAtHeadingPitchAndDistance
        (xcor, ycor, zcor,
  ...
  public double[] right(double delta) {
    ...
    if (StrictMath.abs(sinDelta) < org.nlogo.api.Numbers.Infinitesimal()) {
      sinDelta = 0;
    }
    if (StrictMath.abs(cosDelta) < org.nlogo.api.Numbers.Infinitesimal()) {
      cosDelta = 0;
    }
  ...
  private void turtleMoved(double newX, double newY, double newZ, double oldX, double oldY, double oldZ) {
    ((TieManager3D) _world.tieManager()).turtleMoved(this, newX, newY, newZ, oldX, oldY, oldZ);
  }
  ...
  private void turtleOrientationChanged(double newHeading, double newPitch, double newRoll,
    ...
    ((TieManager3D) _world.tieManager()).turtleOrientationChanged(this, newHeading, newPitch, newRoll,
